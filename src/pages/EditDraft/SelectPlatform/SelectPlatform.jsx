          ...
          return (
            ...
            >
              ...
              />
               ...
               <div className="flex flex-col mx-auto w-full text-3xl font-semibold leading-none text-gray-900 bg-white rounded-3xl p-4 sm:p-5 md:p-6">
  <h2 className="text-lg sm:text-xl md:text-2xl font-semibold text-gray-800 mb-2">
    How do you want to send the Campaign?
  </h2>
  ...
  {maindata.map((item, index) => {
    ...
    return (
      <div
        key={index}
        onClick={() => handleCheckboxChange(item.platformId.type)}
        className="flex flex-wrap gap-2 sm:gap-2 md:gap-3 justify-between items-center px-3 py-1.5 sm:px-4 sm:py-2 md:px-5 md:py-2 mt-2 w-full border border-solid border-blue-950 rounded-full cursor-pointer transition-all duration-300 hover:bg-gray-50"
      >
        <div className="flex gap-2 sm:gap-3 md:gap-4 items-center">
          {selectedOptions[item.platformId.type] ? (
            <img
              ...
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/e3eeb287db08fcbff99ed0ff48250aa0ecbd18b4132c15fb194bfb45fbe97092?placeholderIfAbsent=true&apiKey=d0018788f321472fb76f0852605a7e1f"
              alt="Selected icon"
              className="object-contain shrink-0 rounded-md w-6 sm:w-7 md:w-8 aspect-square"
            />
          ) : (
            <div
              className="flex shrink-0 h-6 sm:h-7 md:h-8 rounded-md border border-solid w-6 sm:w-7 md:w-8"
              role="presentation"
            />
          )}
        ...
        <img
          ...
          src={Icon}
          alt={`${item.platformId.name} icon`}
          className="object-contain shrink-0 w-6 sm:w-7 md:w-8 aspect-square"
        />
      </div>
    );
  })}
