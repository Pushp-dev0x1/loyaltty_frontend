...
const EditDraft = () => {
  ...
  useEffect(() => {
    ...
    setCurrentStep(templatedata?.data.step);
    setmain_url(templatedata?.data.url);
    setCampaignId(templatedata?.data._id);
    const initializeCheckedPlatforms = () => {
      const platforms = templatedata?.data.platformDetails.reduce(
        (acc, platform) => {
          const type = platform.platformId.type;
          if (type in acc) {
            acc[type] = true;
          }
          return acc;
    ...
    initializeCheckedPlatforms();
  }, [templatedata]);

  console.log(templatedata, "main data of template");
  ...
  const [formState, setFormState] = useState({
    ...
    contactno: templatedata?.data.contactno,
        ...
        setPlatformDetails((prevState) => {
          ...
          if (whatsappPlatformIndex === -1) {
            updatedDetails.push({
              ...
              maincontent: whatsappContent.maincontent,
              mainparam: whatsappContent.mainparam,
              parameters: {},
            });
          } else {
        ...
        setPlatformDetails((prevState) => {
          ...
          if (smsPlatformIndex === -1) {
            updatedDetails.push({
              ...
              maincontent: smsContent.maincontent,
              mainparam: smsContent.mainparam,
              parameters: {},
            });
          } else {
    ...
    if (currentStep >= 0 && currentStep <= steps.length - 2) {
      return (
        <MainPreviewTemplate
          ...
          formdata={emailPlatformDetails.parameters}
          themeform={formState}
          whatsappcontent={whatsappContent?.maincontent}
